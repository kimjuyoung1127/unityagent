...
**전체 Phase 완료. 총 63개 write allowlist 명령, 119개 CLI 명령, 12개 MCP 도구 (33→12 통합).**
...
테스트 인벤토리 기준 합계는 **543개**다 (MCP Context Optimization 테스트 포함, Integration 기존 인벤토리 기준).
...
- `doctor`가 plugin source kind (`local-file` / `git`)과 project lock 상태를 함께 보고하도록 확장했다.
