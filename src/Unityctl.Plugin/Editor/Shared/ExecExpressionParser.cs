...
namespace Unityctl.Plugin.Editor.Shared
{
    ...
    public static class ExecExpressionParser
    {
        public static ExecExpression Parse(string expression)
        {
            ...
            {
                var lhs = trimmed.Substring(0, assignmentIndex).Trim();
                var rhs = trimmed.Substring(assignmentIndex + 1).Trim();
            ...
            {
                var target = trimmed.Substring(0, invokeOpenIndex).Trim();
                var argsSegment = trimmed.Substring(invokeOpenIndex + 1, trimmed.Length - invokeOpenIndex - 2);
                var split = SplitTypeMember(target);
                return new ExecExpression
                {
            ...
            var getSplit = SplitTypeMember(trimmed);
            return new ExecExpression
            {
                Kind = ExecExpressionKind.GetMember,
                TypeName = getSplit.TypeName,
                MemberName = getSplit.MemberName
            };
        }
        ...
        private static (string TypeName, string MemberName) SplitTypeMember(string value)
        {
            ...
            var typeName = value.Substring(0, lastDot).Trim();
            var memberName = value.Substring(lastDot + 1).Trim();
        ...
        {
            if (expression.Length < 3 || expression[expression.Length - 1] != ')')
                return -1;
        ...
        private static IReadOnlyList<string> SplitArguments(string arguments)
        {
            ...
            for (var i = 0; i < arguments.Length; i++)
            {
                ...
                parts.Add(TrimArgument(arguments.Substring(start, i - start), i));
                start = i + 1;
            }
            ...
            parts.Add(TrimArgument(arguments.Substring(start), arguments.Length - 1));
            return parts;
        }
